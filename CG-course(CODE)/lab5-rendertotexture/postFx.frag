...
vec3 Mosaic(vec2 coord)
{
	return vec3(textureRect(frameBufferTexture, vec2(floor(coord.x/20) * 20, floor(coord.y/20) * 20))).rgb;

	//return vec3(textureRect(frameBufferTexture, vec2(coord.x - mod(coord.x, 20),  coord.y - mod(coord.y , 20)))).rgb;
}
